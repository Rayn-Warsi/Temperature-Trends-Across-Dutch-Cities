...
ols_estimate <- function(x, y) {
  ...
  list(beta=beta_hat, se=se_beta, t=t_stat, p=p_value,
       ci_beta=c(beta_hat[2] - t_crit * se_beta, beta_hat[2] + t_crit * se_beta),
       yhat=y_hat, upper=y_hat + t_crit * se_fit, lower=y_hat - t_crit * se_fit)
}

run_temperature_analysis <- function(data, dataset_name, year_col="Date",
                                     split_year=1960, start_year=1907, end_year=2024) {
  ...
  locations <- list("De Bilt"=data$De.Bilt, "Eelde"=data$Eelde, "Maastricht"=data$Maastricht)
  ...
  for (loc_name in names(locations)) {
    ...
    cat(sprintf("Slope: %.6f | SE: %.6f | t: %.4f | p: %s\n95%% CI: [%.6f, %.6f]\n",
                m1$beta[2], m1$se, m1$t, format.pval(m1$p), m1$ci_beta[1], m1$ci_beta[2]))
    cat("\n", loc_name, "|", late_label, "\n", sep="")
    cat(sprintf("Slope: %.6f | SE: %.6f | t: %.4f | p: %s\n95%% CI: [%.6f, %.6f]\n",
                m2$beta[2], m2$se, m2$t, format.pval(m2$p), m2$ci_beta[1], m2$ci_beta[2]))
    plot(year, y, pch=16, col="grey40", xlab="Year", ylab="Temperature (°C)",
         main=paste(dataset_name, "-", loc_name), cex.main=1.1)
    ...
    legend("topleft", legend=c("Observed", paste("Trend", early_label), paste("CI", early_label),
                                paste("Trend", late_label), paste("CI", late_label)),
           col=c("grey40","blue","blue","red","red"), pch=c(16,NA,NA,NA,NA),
           lty=c(NA,1,2,1,2), lwd=c(NA,2,1,2,1), bty="n", cex=0.7)
  }
  par(mfrow=c(1,1))
}
...
run_temperature_analysis(data_annual, "Annual Data", split_year=1960, end_year=2024)
run_temperature_analysis(data_monthly, "Smoothed Monthly Data", split_year=1960, end_year=2024)